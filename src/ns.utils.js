...
Parser.prototype.start = function(input, id) {
    this.input = input;
    this.p = 0;
...
Parser.prototype.parse = function(id, params) {
...
Parser.prototype.la = function(n) {
...
var parser = new Parser(rules, tokens);

var _cache = {};

var jpath = function(expr, data, vars, funcs) {
    return jpath.toScalar(jpath.expr(expr)(data, vars, funcs));
};

jpath.raw = function(expr, data, vars, funcs) {
    return jpath.expr(expr)(data, vars, funcs);
};

jpath.scalar = function(expr) {
    var compiled = jpath.expr(expr);

    return function(data, vars, funcs) {
        return jpath.toScalar(compiled(data, vars, funcs));
    };
};

jpath.boolean = function(expr) {
    var compiled = jpath.expr(expr);

    return function(data, vars, funcs) {
        return jpath.toBoolean(compiled(data, vars, funcs));
    };
};

jpath.string = function(str) {
...
jpath.toScalar = function(result) {
...
jpath.toBoolean = function(result) {
...
function compileObject(obj) {
    ...
    return function(data, vars, funcs) {
        ...
        for (var key in items) {
            r[key] = jpath.toScalar(items[key](data, vars, funcs));
        }
...
function compileArray(arr) {
    ...
    return function(data, vars, funcs) {
        ...
        for (var i = 0; i < l; i++) {
            r.push(jpath.toScalar(items[i](data, vars, funcs)));
        }
...
function ast2js(ast, exprs) {
    ...
    switch (ast._id) {
        ...
        case 'jpath':
            p = jpath2func(ast, exprs);
            js = 't' + p + '(node, root, vars, funcs)';
            break;

        case 'filter':
            p = jpath2func(ast.jpath, exprs);
            js = 't' + p + '(new ns.JNode(vars["' + ast.name + '"]), root, vars, funcs)';
            break;
    }
...
jpath.set = function(jpath, data, value) {
...
var utils = {
     ...
     */
    inherit: function(ctor, base, mixin) {
        var F = function() {};
        F.prototype = base.prototype;
        var proto = ctor.prototype = new F();

        if (mixin) {
            utils.extend(proto, mixin);
        }
