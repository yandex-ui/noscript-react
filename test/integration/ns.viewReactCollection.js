describe('ns.ViewCollection', function() {
    ...
    describe('события видов в коллекции', function() {
        describe('простая коллекция', function() {
            beforeEach(function() {
                ns.Model.define('m-collection', {
                    split: {
                        items: '.items',
                ...
                this.events = {
                    'ns-view-htmlinit': this.sinon.spy(),
                    'ns-view-show': this.sinon.spy(),
                    'ns-view-hide': this.sinon.spy(),
                    'ns-view-htmldestroy': this.sinon.spy()
                };
            ...
            describe('должны всплывать на каждой вьюшке', function() {
                it('ns-view-htmlinit', function() {
                    expect(this.events['ns-view-htmlinit'].callCount).to.be.eql(
                        ns.Model.get('m-collection').get('.items').length
                    );
                });
                it('ns-view-show', function() {
                    expect(this.events['ns-view-show'].callCount).to.be.eql(
                        ns.Model.get('m-collection').get('.items').length
                    );
                });
            });

            describe('после добавления вьюшки в коллекцию', function() {
                beforeEach(function() {
                    var item = ns.Model.get('m-collection-item', {
                        id: 2
                    });
                    item.setData({ id: 2 });
                    ns.Model.get('m-collection').insert(item);
                ...
                describe('должны всплывать еще по разу', function() {
                    it('ns-view-htmlinit', function() {
                        expect(this.events['ns-view-htmlinit'].callCount).to.be.eql(1);
                    });
                    it('ns-view-show', function() {
                        expect(this.events['ns-view-show'].callCount).to.be.eql(1);
                    });
                });
            });

            describe('после удаления вьюшки из коллекции', function() {
                beforeEach(function() {
                    ns.Model.get('m-collection').remove(
                        ns.Model.get('m-collection-item', { id: 1 })
                    );
                    return new ns.Update(this.APP, ns.layout.page('app', {}), {}).render();
                });

                describe('должны всплывать', function() {
                    it('ns-view-hide', function() {
                        expect(this.events['ns-view-hide'].callCount).to.be.eql(1);
                    });
                    it('ns-view-htmldestroy', function() {
                        expect(this.events['ns-view-htmldestroy'].callCount).to.be.eql(1);
                    });
                });
            });
        });

        describe('с вложенными видами в элементы коллекции', function() {
            beforeEach(function() {
                ns.Model.define('m-collection', {
                    split: {
                        items: '.items',
                ...
                ns.layout.define('l-collection-item', {
                    'v-collection-item': 'v-collection-item-inner'
                });
                ...
                ns.ViewReactCollection.define('v-collection', {
                    ...
                    split: {
                        byModel: 'm-collection',
                        intoLayouts: function() {
                            return 'l-collection-item';
                        }
                    }
                });
                ns.ViewReact.define('v-collection-item', {
                ...
                ns.ViewReact.define('v-collection-item-inner', {
                    events: this.events
                });
                this.APP = ns.View.create('app');
            ...
            describe('при первой отрисовке должны всплывать', function() {
                it('ns-view-htmlinit', function() {
                    expect(this.events['ns-view-htmlinit'].callCount).to.be.eql(
                        ns.Model.get('m-collection').models.length * 2
                    );
                });
                it('ns-view-show', function() {
                    expect(this.events['ns-view-show'].callCount).to.be.eql(
            ...
            describe('при удалении вьюшки из коллекции', function() {
                beforeEach(function() {
                    ns.Model.get('m-collection').remove(
                        ns.Model.get('m-collection-item', { id: 2 })
                    );
                    return new ns.Update(this.APP, ns.layout.page('app', {}), {}).render();
                });

                it('ns-view-htmldestroy', function() {
                    expect(this.events['ns-view-htmldestroy'].callCount).to.be.eql(2);
                });
                it('ns-view-hide', function() {
                    expect(this.events['ns-view-hide'].callCount).to.be.eql(2);
                });
            });
        });
    });
